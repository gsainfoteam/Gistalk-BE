...
export class ScoringService {
    ...
    async getLectureInfo(lecture_id : number): Promise<any>
    {
        ...
        if(!found)
        {
            throw new NotFoundException(`ID에 할당된 강의가 없습니다 ID : ${lecture_id}`);
        }
        else
        {
    ...
    async scoring(main_lecture_id : number): Promise<any>
    {
        ...
        if(found)
        {
            throw new NotFoundException(`해당 강의는 이미 최소 1번 이상 계산되었습니다`);
        }
        if(people)
        {
    ...
    async scoringFuntion(type : string, lecture : any, people : number, main_lecture_id : number)
    {
        var i : number;
        let diff_sum : number = 0;
        let stren_sum : number = 0;
        let help_sum : number = 0;
        let inter_sum : number = 0;
        let lots_sum : number = 0;
        let sati_sum : number = 0;
        ...
        total_score = (diff_sum + stren_sum + help_sum + inter_sum + lots_sum + sati_sum )/6;
